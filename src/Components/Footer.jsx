...
import { SERVICES, BUSINESS, PHONE, TEL_HREF, reportCallConversion } from "../config";
...
export default function Footer() {
  ...
  const navItems = [
    ...
    { label: "Get a Free Quote", to: { pathname: "/", hash: "#contact" } },
  ];
    ...
    <footer className="bg-white border-t border-gray-200">
      ...
      <div className="bg-yellow-400 py-5 px-4">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <a
            ...
            className="bg-black hover:bg-zinc-800 text-white font-bold text-sm px-6 py-3 min-h-[48px] rounded-xl transition-all duration-200 flex items-center gap-2 whitespace-nowrap"
          >
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 flex flex-row flex-wrap items-start justify-between gap-8">
        ...
        <div className="flex flex-col gap-3">
          <Link to="/" className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <div className="text-black font-bold text-sm">{BUSINESS.name}</div>
              <div className="text-yellow-500 text-xs font-semibold tracking-widest uppercase">{BUSINESS.region}</div>
            </div>
          </Link>
        ...
        <div className="flex flex-col">
          ...
          {navItems.map((item) => (
            <Link
              key={item.label}
              to={item.to}
              className="text-gray-500 hover:text-yellow-500 text-sm transition-colors duration-200 min-h-[48px] flex items-center"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex flex-col">
          ...
          <a
            href={TEL_HREF}
            onClick={reportCallConversion}
            className="text-gray-500 hover:text-yellow-500 text-sm transition-colors min-h-[48px] flex items-center"
          >
            {PHONE.display}
          </a>
          <span className="text-gray-500 text-sm min-h-[48px] flex items-center">{BUSINESS.city}</span>
          <span className="text-gray-500 text-sm min-h-[48px] flex items-center">Available 24/7</span>
        </div>
      </div>
      ...
      <div className="border-t border-gray-100 py-4 px-4">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-1 text-xs text-gray-400">
          <p>© {currentYear} {BUSINESS.name} {BUSINESS.region}. All rights reserved.</p>
